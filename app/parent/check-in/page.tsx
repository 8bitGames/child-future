...
// Helper to convert between CheckIn types
function convertToWeeklyCheckIn(checkIn: CheckIn): WeeklyCheckIn {
  const dateStr = checkIn.date || checkIn.weekStart || checkIn.createdAt;
  return {
    id: checkIn.id,
    weekStart: dateStr,
    weekEnd: checkIn.weekEnd || dateStr,
    childData: checkIn.childData || {
      activityLevel: null,
      favoriteActivity: '',
      weeklyMood: (checkIn.mood as WeeklyCheckIn['childData']['weeklyMood']) || null,
      freeNote: checkIn.notes || ''
    },
    parentData: checkIn.parentData || {
      noticeableChanges: '',
      completedActivities: [],
      specialEpisode: ''
    },
    createdAt: dateStr,
    updatedAt: checkIn.updatedAt || dateStr
  };
}

function convertToCheckIn(weeklyCheckIn: WeeklyCheckIn): CheckIn {
  return {
    id: weeklyCheckIn.id,
    date: weeklyCheckIn.weekStart,
    weekStart: weeklyCheckIn.weekStart,
    weekEnd: weeklyCheckIn.weekEnd,
    mood: weeklyCheckIn.childData.weeklyMood || undefined,
    notes: weeklyCheckIn.childData.freeNote || undefined,
    childData: weeklyCheckIn.childData,
    parentData: weeklyCheckIn.parentData,
    createdAt: weeklyCheckIn.createdAt,
    updatedAt: weeklyCheckIn.updatedAt
  };
}
...
export default function CheckInPage() {
  ...
  useEffect(() => {
    ...
    // Get latest result for the selected child
    const result = getChildLatestResult(id);
    setLatestResult(result);

    // Get check-ins for the selected child and convert to WeeklyCheckIn format
    const storedCheckIns = getChildCheckIns(id);
    setCheckIns(storedCheckIns.map(convertToWeeklyCheckIn));
    setLoading(false);
  }, [router]);
  ...
  const handleSave = () => {
    ...
    // Save to child-specific storage
    saveChildCheckIns(childId, updatedCheckIns.map(convertToCheckIn));
    alert('체크인이 저장되었습니다!');
  };
