...
export default function ReportPage() {
  ...
  useEffect(() => {
    ...
    const childReports = getChildReports(id);

    setResults(childResults);
    // Convert storage types to local types with defaults
    setDiaryEntries(childDiary.map(d => ({
      id: d.id,
      date: d.date || d.createdAt,
      activity: d.activity || { title: '' },
    ...
    setCheckIns(childCheckIns.map(c => ({
      ...
      childEvaluation: {
        ...
        activityLevel: c.childData?.activityLevel || ''
      },
      parentObservation: {
        noticeableChanges: c.parentData?.noticeableChanges || '',
        completedActivities: c.parentData?.completedActivities || []
      }
    })));
    if (childReports.length > 0) {
      const latestReport = childReports[0]; // Reports are stored newest first
      setReport({
        summary: latestReport.content,
        strengths: [],
        growthAreas: [],
        recommendations: [],
        parentTips: [],
        generatedAt: latestReport.timestamp,
      });
    }
  }, [router]);
  ...
  const generateReport = async () => {
    if (!latestResult || !childId) return;
    ...
    try {
      ...
      const aiReport: AIReport = {
        id: `report-${Date.now()}`,
        title: `${selectedPeriod === 'monthly' ? '월간' : selectedPeriod === 'quarterly' ? '분기' : '연간'} 성장 리포트`,
        content: newReport.summary,
        summary: newReport.summary.slice(0, 100),
        timestamp: newReport.generatedAt,
      };
      addChildReport(childId, aiReport);
    } catch (err) {
  ...
  const generateLocalReport = () => {
    ...
    const aiReport: AIReport = {
      id: `report-${Date.now()}`,
      title: `${selectedPeriod === 'monthly' ? '월간' : selectedPeriod === 'quarterly' ? '분기' : '연간'} 성장 리포트 (로컬)`,
      content: localReport.summary,
      summary: localReport.summary.slice(0, 100),
      timestamp: localReport.generatedAt,
    };
    addChildReport(childId, aiReport);
  };
