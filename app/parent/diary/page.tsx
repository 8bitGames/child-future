...
import {
  ...
  getChildDiary,
  saveChildDiary,
  DiaryEntry as StoredDiaryEntry,
} from '@/lib/utils/child-storage';
import { IBProfile, IB_PROFILE_NAMES, AssessmentResult } from '@/lib/types/result';
import {
  ArrowLeft,
  BookOpen,
  Plus,
...
export default function DiaryPage() {
  ...
  useEffect(() => {
    ...
    // Load diary entries for the selected child
    const savedEntries = getChildDiary(id);
    setEntries(savedEntries.map(e => ({
      id: e.id,
      date: e.date || e.createdAt,
      activity: e.activity || { title: '', description: '', photos: [] },
      childReflection: e.childReflection || e.content || '',
      parentNote: e.parentNote,
      ibProfile: e.ibProfile as IBProfile | undefined,
      badge: e.badge,
  ...
  const saveEntries = (newEntries: DiaryEntry[]) => {
    if (!childId) return;
    // Convert to StoredDiaryEntry format and save
    const storedEntries: StoredDiaryEntry[] = newEntries.map(e => ({
      id: e.id,
      date: e.date,
      activity: e.activity,
      childReflection: e.childReflection,
      parentNote: e.parentNote,
      ibProfile: e.ibProfile,
      badge: e.badge,
      createdAt: e.createdAt,
      updatedAt: e.updatedAt
    }));
    saveChildDiary(childId, storedEntries);
