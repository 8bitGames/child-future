...
interface ChildLoginClientProps {
  initialCode?: string;
}

export function ChildLoginClient({ initialCode }: ChildLoginClientProps) {
  ...
  // Check for code from props on mount
  useEffect(() => {
    if (initialCode) {
      const normalizedCode = parseFamilyCode(initialCode);
